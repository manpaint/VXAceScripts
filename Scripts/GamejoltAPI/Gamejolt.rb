...
#Note from Manpaint: This script was heavily modified.
...
module GameJolt
  ...
  def self.submit_score(score, score_description = "(SCORE) point(SCOREPLURAL)", scoreboard_id = "main", extra_data = "", allow_guests = false, guest_name = "")
    ...
    if extra_data != ""
      ...
      extra_data = "$game_actors[11].name.to_s + $game_actors[12].name.to_s + $game_actors[13].name.to_s"
    end
    if isloggedin
...
#          guest_name.gsub!("&", "_")
           File.rename('System/Config/0x122=1', 'System/Config/0x122=0')
        end
        if guest_name == ""
          return false
        else
