...
#------------------------------------------------------------------------------#
   ...
   REGION_EFFECT_SWITCH = 16   # Turn this switch ON to disable the effects.

   MAX_EFFECTS = 20           # Number of effects that can be on screen
                              # before being removed. (To prevent lag)
                               
   SPAWN_MAP_ID = 96           # Map ID of the map you store event effects in.
...
#------------------------------------------------------------------------------#
    ...
    1 => ["", 0, 0, 5, 0],              # Dirt dust
    2 => ["Blow7", 0, 150, 0, 0],      # Wood surface noise only
    3 => ["Water1", 0, 110, 1, 0],     # Shallow Water
    
    18 => ["Earth2", 0, 130, 0, 0],     # Hard surface noise only
    5 => ["1x07", 60, 100, 2, 0],    # Footprints /snow
    6 => ["Ice9", 0, 130, 3, 0],       # Walking over thick grass
    7 => ["", 0, 0, 0, 1],              # Calls common event 1 only
    8 => ["Fire3", 0, 120, 4, 2],      # Calls fire trap
    9 => ["0x142", 20, 100, 0, 0],      # Rock path surface
    10 => ["1x01", 60, 100, 0, 0],      # Grass path surface
    11 => ["1x06", 70, 120, 0, 0],      # Wood path surface
    12 => ["1x08", 60, 100, 2, 0],    # Footprints /sand
    37 => ["", 0, 120, 6, 0],          # Dust cloud
