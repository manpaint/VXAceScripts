...
module YEA
  module SAVE
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    MAX_FILES = 28         # Maximum saves a player can make. Default: 16
    if File.file?('default_langset=2')
    SLOT_NAME = "Svg. %s"  # How the file slots will be named.
    else    
    SLOT_NAME = "Save %s"  # How the file slots will be named.  
    end
    # These are the icons
    SAVE_ICON  = 1       # Icon used to indicate a save is present.
    EMPTY_ICON = 2       # Icon used to indicate an empty file.
    ...
    if File.file?('default_langset=2')
      ...
      ACTION_DELETE = "Supprimer"         # Text used for deleting games.
      SELECT_HELP = "Sélectionnez une sauvegarde."
      LOAD_HELP   = "Charger cette sauvegarde."
      SAVE_HELP   = "Sauvegarder la progression."
      DELETE_HELP = "Supprimer les données de cette sauvegarde."
      EMPTY_TEXT = "Aucune donnée de sauvegarde"      # Text used when no save data is present.
      PLAYTIME   = ""          # Text used for total playtime.
      TOTAL_SAVE = "Nb de sauvegardes: "     # Text used to indicate total saves.
    ...
    #ACTION_LOAD   = ($game_actors[60].name)          # Text used for loading games.
    #ACTION_LOAD   = ($game_variables[1])          # Text used for loading games.
    #ACTION_LOAD   = "Load" 
          # Text used for loading games.
